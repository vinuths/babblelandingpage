...
const Barchart = ({branchesCompany}) => {
    ...
    const statewiseCounts = useSelector((state) => state.regCountReducer.auditData1 || {});
    const branches = useSelector((state) => state.regCountReducer.branches || []); // Get branches from store
    const loading = useSelector((state) => state.regCountReducer.loadingcompstatus);
    const error = useSelector((state) => state.regCountReducer.error);
...
,[CompanyBranchesGet])
    ...
    useEffect(() => {
        ...
        dispatch(auditRegCountAll(postBody));
    ...
    return (
        <div className="chart-container">
            {/* Heading */}
            <h2 className="chart-heading">Registrations/Licenses/Permissions</h2>
