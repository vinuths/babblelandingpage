...
import { stateGets, minimumWagesLibraryDelete, minimumWagesLibraryPaginatedGet } from "../../../../store/actions/otherActions";
import { EditOutlined, DeleteOutlined, EyeOutlined } from "@ant-design/icons";
import { Link } from "react-router-dom";
import Swal from 'sweetalert2';
import Popup from "../../../../components/Popup";
import moment from "moment";
import { updateMinimumWagesLibraryStatus } from "../../../../routes/api";
...
const MinWagesViewTable = ({ localPage, setLocalPage }) => {
    const dispatch = useDispatch();
    
    const { data, totalCount, loading } = useSelector(
        (state) => state.minimumWagesLibraryPaginatedRed
    );
    // console.log("minimumWagesLibraryPaginatedRed",data);
    ...
    const fetchData = (page = localPage) => {
        ...
        dispatch(minimumWagesLibraryPaginatedGet({ page, limit: pageSize, filters }));
    };
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            await dispatch(minimumWagesLibraryDelete(id));
            fetchData(localPage);
    ...
    const openInPopupForUpdate = (item) => {
        ...
        setOpenPopup(true);
        setPageTitle('Edit Minimum Wage E-Library');
        setModalWidth('400px');
    };
    ...
    const handleStatusToggle = async (id, currentStatus) => {
        try {
            await updateMinimumWagesLibraryStatus(id, !currentStatus);
    ...
    return (
        <div className="container-fluid">
            ...
            <div className="table-responsive">
                {loading ? (
                    ...
                    <div style={{ backgroundColor: 'whitesmoke', borderRadius: '8px', textAlign: 'center', paddingTop: '25px', height: '100px' }}>
                        <h1 style={{ color: 'darkgray', fontStyle: 'italic' }}>No Minimum Wages E-Library Available</h1>
                    </div>
                )}
            </div>
            ...
            <Popup openPopup={openPopup} pageTitle={pageTitle} setOpenPopup={setOpenPopup} modalWidth={modalWidth}>
                {openPopup && <MinWagesViewCreate addOrEdit={addOrEdit} recordForEdit={recordForEdit} setLocalPage={setLocalPage} />}
            </Popup>
        </div>
    );
};
