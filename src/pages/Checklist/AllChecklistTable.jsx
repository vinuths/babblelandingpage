...
const AllChecklistTable = () => {
  ...
  const { loadingall, checklistInfoAll, totalPages, currentPage } = getChecklistall;
  ...
  useEffect(() => {
    ...
    dispatch(checklistGetAll(page));
    dispatch(companyTableGet());
  }, [dispatch, page])

 
  const handlePageChange = (page, pageSize) => {
    dispatch(checklistGetAll(page, pageSize));
  ...
  const getBbranch = (company, state) => {
    const elementcompanybranch = myElementRefCompany.current;
    const elementcompanyState = myElementRefState.current;
    const postBody = {
      id: elementcompanybranch.value,
      state: elementcompanyState.value || ''
    }
    dispatch(branchGet(postBody));
  }
    ...
    <React.Fragment>
      <div className="row">
        <div className="col-md-4 col-lg-15 mb-2 mb-lg-3 mb-md-3 ">
          <select className="form-select" ref={myElementRefCompany} aria-label="Default select example" id="company" name="company" value={company} onChange={(e) => { setCompany(e.target.value); filter(); const selectedCompany = e.target.value; getBbranch(selectedCompany, state); }} hidden>
        ...
        <div className="col-md-4 col-lg-15 mb-2 mb-lg-3 mb-md-3">
          <select className="form-select" ref={myElementRefState} aria-label="Default select example" id="state" name="state" value={state} onChange={(e) => { setState(e.target.value); filter(); const selectedState = e.target.value; getBbranch(company, selectedState); }} hidden>
            ...
            <div className="table-responsive">
              ...
              {showTable1 ? (
                <Table
                  ...
                  pagination={{
                    current: currentPage, // Ensure it updates from Redux state
                    total: totalPages * pageSize, // Reflect API response
                    ...
                    onChange: handlePageChange, // Handles page & pageSize change
                  }}
                  scroll={{ x: 1250 }}
                  sticky
                />) : (
